...
import numpy as np
import pandas as pd
from DataAPI import get_trading_days, get_next_trading_day, \
    get_universe, convert_to_date, get_stock_daily_prices, get_stock_daily_return, get_last_trading_day
...
def get_ret(start, end):
    # r = get_feature('origin',start,end,corr_info=['close_re'])['close_re']
    days = get_trading_days(start, end)
    df_list = []
    for date in days:
        # change 2021.09.02 for close or open issue...
        # temp = get_stock_daily_return(date, ti=0, period='1d')
        temp = get_stock_daily_prices(get_next_trading_day(date), fields=['pct_chg'])
        if temp is not None:
            temp.reset_index(inplace=True)
            temp.rename(columns={"pct_chg": "1d_ret", "timestamp": "date"}, inplace=True)
            temp['date'] = int(datetime.datetime.strftime(date, '%Y%m%d'))
            df_list.append(temp)
    df = pd.concat(df_list)
...
def cal_barra_attribution(start_date, end_date, universe, file, file_type='series',
    ...
    if file_type == 'series':
        ...
        B.calc_top(Top)
        if output is not None:
            output1 = str(output) + f'Portfolio_factor_attribution.csv'
            output2 = str(output) + f'Portfolio_excess_return_decomposition.csv'
        else:
            output1, output2 = None, None
        B.make_table_attr(output1)
        B.make_table_deco(output2)
...
def test():
    path = r"/home/liguichuan/Desktop/Project/Release/Platform/backtest/metrics/demo/sample.pickle"
    file = pd.read_pickle(path)
    B = BarraTools()
    B.load_data(start=20170103, end=20170428, universe='investable',
                file=file, file_type='series', index='ZZ500')
    B.calc_top(top=200)
    B.make_table_attr()
    B.make_table_deco()
...
def test_1():
    ...
    # path = r"/home/liguichuan/Desktop/Project/Release/Platform/backtest/metrics/demo/sample.pickle"
    # file = pd.read_pickle(path)
    path = r'/home/ShareFolder/lh/Pos.parquet'
    file = pd.read_parquet(path)
    # file = pd.read_pickle(path)[['1']]
    data = cal_barra_attribution(20210604, 20210730, universe='ZZ500', file=file,
