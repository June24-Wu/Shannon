...
class Alpha(object):
    ...
    def __init__(self, alpha_name=None, logger=None, **kwargs):
        if isinstance(alpha_name, str):
            self.alpha_name = [alpha_name]
        elif isinstance(alpha_name, list):
            pass
        elif alpha_name is None:
            pass
        else:
            raise ValueError(f'Unrecognized alpha name type: {type(alpha_name)}!')
    ...
    def create(self, start_time, end_time, output=None, mode='prod'):
        result_dict = dict()
        for factor in self.alpha_name:
            result_dict[factor] = getattr(self, f"generate_{factor}")(start_time, end_time, output=output,
                                                                      mode=mode)
        return result_dict


class pv_liguichuan_degree(Alpha):
    ...
    def initializeData(self, start_time, end_time):
        num_count = pv_liguichuan_degree.update_data_count["DayilyPrices"]
        while num_count > 0:
    ...
    def generate_pv_liguichuan_degree(self, start_time, end_time, output=None, mode='dev'):
        alpha_name = "pv_liguichuan_degree"
...
if __name__ == '__main__':
    alpha = pv_liguichuan_degree("pv_liguichuan_degree")
    alpha.create(start_time='20170101', end_time='20180101', output='/home/liguichuan/桌面/features', mode='dev')
