...
max_workers = 24
columns_used = ['timestamp', 'ticker', '1m_ret', '5m_ret', '15m_ret', '30m_ret', '60m_ret', '120m_ret', '1d_ret',
                '2d_ret', '3d_ret', '4d_ret', '5d_ret', '10d_ret', '20d_ret']
...
def handler_close(data):
    ...
    if return_period * frequency >= 241:
        df_return.columns = [f'{return_period * frequency // 241}d_ret']
    else:
...
def handler_output(data):
    ...
    df['trading_status'] = np.where(np.logical_and(np.logical_and(
        ...
        0, df['trading_status'])
    df['trading_status'] = np.where(np.logical_and(df['trading_status'] == 2, df.index.get_level_values(0).hour == 15),
                                    0, df['trading_status'])
...
def get_return_data(df, trade_days, method, forced=False, period=0, df_close_all=None):
    ...
    return_list = [1, 5, 15, 30, 60, 120, days_return * 1, days_return * 2, days_return * 3, days_return * 4,
...
def run():
    ...
    start_date, end_date = '2021-11-01', '2022-12-15'
    method_list = ['vwap']  # support open, close, twap, vwap
    day_length = 30
    periods = [31]
    days_count = 20

    assert isinstance(frequency, int), 'frequency cannot be assign with value: {}'.format(frequency)
